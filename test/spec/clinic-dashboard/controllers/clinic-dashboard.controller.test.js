	...
	describe('Controller: ClinicDashboardCtrl', function () {
		...
		beforeEach(inject(function ($controller, $injector, _$stateParams_, $rootScope) {
			...
			*/
			...
			controller = $controller('ClinicDashboardCtrl', { $rootScope: $rootScope, $scope: controllerScope, $stateParams: stateParams, OpenmrsRestService: OpenmrsRestService, LocationModel: locationModel });
		...
		it('should switch the active tab by changing the activeTabId when switchTabByIndex is invoked', function () {
			controllerScope.switchTabByIndex(1);
			expect(controllerScope.activeTabId).to.equal(1);
			
			controllerScope.switchTabByIndex(2);
			expect(controllerScope.activeTabId).to.equal(2);
		});
