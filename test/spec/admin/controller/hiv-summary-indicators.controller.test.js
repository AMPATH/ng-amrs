...
(function () {
  'use strict';
  describe('Controller: HivSummaryIndicatorsCtrl ', function () {
    var controller;
    var scope;
    var filterFilter;
    var state;
    ...
    beforeEach(inject(function ($controller, $injector, $rootScope, OpenmrsRestService, EtlRestService, moment) {
      ...
      controller = $controller('HivSummaryIndicatorsCtrl', {
        $rootScope: scope,
        $scope: scope,
    ...
    it('should Filter variables needed for the report generation exists and they have correct data types', function () {
      ...
      expect(scope.selectedLocation).to.exist;
      expect(scope.selectedIndicatorBox).to.exist;
      //Make sure data types are enforced
      expect(scope.startDate).to.be.an('date');
      expect(scope.endDate).to.be.an('date');
    });

    it('should ensure loadHivSummaryIndicators method is defined and it is a function', function () {
      expect(scope.loadHivSummaryIndicators).to.exist;
      expect(scope.loadHivSummaryIndicators).to.be.an('function');
    });
