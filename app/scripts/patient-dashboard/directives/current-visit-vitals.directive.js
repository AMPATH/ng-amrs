...
(function() {
  ...
  angular
    ...
    .directive('currentVisitVitals', directive);
  ...
  function directive() {
    return {
      ...
      templateUrl: 'views/patient-dashboard/current-visit-vitals.html',
  ...
  function currentVisitController($scope, $rootScope, vService, $stateParams,
    encService, encModel, todayVitalModel, $filter, $timeout,
    $location, dialogs) {
    ...
    function __fetchVisitCompletedEncounters(visitUuid, tryCount) {
      ...
      vService.getVisitEncounters(visitUuid, function(visitEncounters) {
        if (visitEncounters.length > 0) {
          ...
          var vitals = [];
          for (var i = 0; i < visitEncounters.length; i++) {
            var encounter = visitEncounters[i];
            if (encounter.encounterType.uuid === 'a44ad5e2-b3ec-42e7-8cfa-8ba3dbcf5ed7') {
              vitals.push(todayVitalModel.getVitalsFromObs(encounter.obs));
              break;
            }

          }

          $scope.currentVisit.vitals = vitals;
        }
