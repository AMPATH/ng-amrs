...
(function() {
  ...
  angular
    ...
    .directive('moh731ClinicDashboardReportFilters', directive);
  ...
  function directive() {
    return {
      ...
      templateUrl: "views/clinic-dashboard/moh-731-report-filter-controls.html"
    };
  }

  moh731ReportFilterController.$inject = ['$scope', '$rootScope', 'SearchDataService',
    'moment', '$state', '$filter', 'CachedDataService', 'OpenmrsRestService',
    'LocationModel', 'ClinicDashboardService'
  ];

  function moh731ReportFilterController($scope, $rootScope, SearchDataService,
    moment, $state, $filter, CachedDataService, OpenmrsRestService,
     LocationModel, ClinicDashboardService) {
    $scope.forms = [];
    $scope.selectedForms = {};
    ...
    $rootScope.$on('location:change', function(){
      ...
      $scope.$parent.selectedSearchLocations.push(ClinicDashboardService.
